...
def main():
    ...
    while True:
        with open("dataset_research/workspace/data_transmit/signal.json", "r") as f:
            signal = json.load(f)["signal"]
        if signal == 0:
            time.sleep(10)
        else:
            with open("dataset_research/workspace/data_transmit/data.json", "r") as f:
                data = json.load(f)
            query = data["query"]
            top_k = data["top_k"]
            print(query, top_k)
            results = rag.search(query, top_k, search_method=args.method)
            with open("dataset_research/workspace/data_transmit/data.json", "w") as f:
                json.dump({"result":results},f,indent=4)
            with open("dataset_research/workspace/data_transmit/signal.json", "w") as f:
                json.dump({"signal":0},f,indent=4)
